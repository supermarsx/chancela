 ...
 * (`a1b2c3d4…e5f6a7b8`), with the FULL value on the native `title` tooltip and an
 * optional click-to-copy control (t13 item 5). Short values are shown whole. The
 * abbreviation is display-only — the complete value is always one hover or copy away,
 * so none of the evidence is hidden.
 */
...
export function Digest({ value, edge = 8, copyable = true }: DigestProps) {
  ...
  return (
    <span className="digest">
      <code className="mono digest__value" title={value}>
      ...
      {copyable ? (
        <button
          ...
          onClick={copy}
          title={copied ? t('common.copied') : t('ui.digest.copy')}
