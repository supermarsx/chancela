...
export const providerCredentialsPtPT = {
  ...
  // Which environment the run will use, stated BEFORE the operator commits (t113). The entry's
  // own `env` selector is authoritative on the server, so it is what is read here.
  'providerCredentials.cmdTest.envEntryProd':
    'Esta credencial está marcada como «Produção», por isso o teste corre contra o serviço de produção da AMA.',
  'providerCredentials.cmdTest.envEntryPreprod':
    'Esta credencial está marcada como «Pré-produção». A assinatura de teste só corre em produção, por isso será recusada: mude o ambiente desta credencial antes de continuar.',
  'providerCredentials.cmdTest.envEntryInherited':
    'Esta credencial não indica ambiente, por isso usa o ambiente predefinido nas definições de assinatura. O teste só corre se esse valor for «Produção».',
  ...
  'providerCredentials.cmdTest.selfValidationOk':
    'A aplicação verificou a assinatura que produziu.',
...
export const providerCredentialsEnglish = {
  ...
  'providerCredentials.cmdTest.permission':
    'Requires the signing.configure and signing.perform permissions.',
  ...
  'providerCredentials.cmdTest.envEntryProd':
    'This credential is marked "Production", so the test runs against the AMA production service.',
  'providerCredentials.cmdTest.envEntryPreprod':
    'This credential is marked "Pre-production". The test signature only runs in production, so it will be refused: change the environment on this credential before continuing.',
  'providerCredentials.cmdTest.envEntryInherited':
    'This credential selects no environment, so it uses the default in the signing settings. The test only runs if that default is "Production".',
