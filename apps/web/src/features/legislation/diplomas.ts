 ...
 *  - `'summary'` — the `extract` faithfully *describes* the provision in the app's own
...
export const LEGISLATION_THEMES = [
...
export type LegislationTheme = (typeof LEGISLATION_THEMES)[number]['id'];

/** Whether an extract is reproduced verbatim (`quote`) or faithfully described (`resumo`). */
export type ExtractKind = 'quote' | 'summary';
...
export interface Diploma {
  ...
  extract: string;
  /** `'quote'` = word-for-word; `'summary'` = faithful description (no quotation marks). */
...
export function diplomasByTheme(theme: LegislationTheme): Diploma[] {
  return DIPLOMAS.filter((d) => d.theme === theme);
}
