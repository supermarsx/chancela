...
- Signing DSS/trust-validation checkpoint: `63df508` is the signing validation
  implementation checkpoint inside the current `baf9f41` snapshot. Static markers pin
...
annotation and remedy-classification evidence plus the current `baf9f41` snapshot's privacy
