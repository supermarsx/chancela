...
- Signing DSS/trust-validation checkpoint: `63df508` is the signing validation
  implementation checkpoint inside the current `cf4a157` snapshot. Static markers pin
...
annotation and remedy-classification evidence plus the current `cf4a157` snapshot's privacy
