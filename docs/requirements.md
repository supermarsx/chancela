...
`2.0` CPU / `1 GB`, Postgres at `2.0` CPU / `1 GB`, Redis at `1.0` CPU / `320 MB`.
...
| PostgreSQL | 16 (`postgres:16-alpine`) | `postgres` profile only; pinned by digest in the hardened compose. |
| Redis | 7 (`redis:7-alpine`) | `postgres` profile only, cache-aside; pinned by digest in the hardened compose. |
