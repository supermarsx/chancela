...
*Updated 2026-07-10 through committed implementation snapshot `a168af37a5e3edad565cd412efa1e4f33bf6a639`,
...
handoff import UI. Earlier coverage text remains prior snapshot context. All top-level spec areas remain **PARTIAL**.
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, with
  ...
  migration/export-restore and executable key rotation tooling beyond the current key-ops
  guard/plan plus web/API/store preflight and store-level rekey evidence, and
...
- Database key-ops status/preflight is a secret-free configuration/build/header classification and
  startup guard with web/API/store key-rotation preflight/rekey evidence. It does not prove live
