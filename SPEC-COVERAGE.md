...
- **MCP tool coverage:** The stdio MCP catalog includes read/write-controlled `/api/v1` tools for
  ...
  gaps, and a local chronology review summary resource for deterministic caller-supplied
  event-kind/status/date-range/evidence-marker counts; all remain static/offline or local
  deterministic review aids with no provider calls, legal-validity claim, registry-certification
  claim, ownership determination, or AI-completion claim.
...
- MCP draft human-verification states are exposed in output envelopes, and AI-origin act drafts now
  ...
  comparison/summary signals with no bridge/API/provider calls, no secrets, and
  no raw text/contact/secret/access-code echo, no legal,
