...
PDF/UA report version 10 scoped table-header evidence,
...
- Working tree keeps Documents/CI **PARTIAL**: PDF accessibility report JSON
  version 10 now includes scoped row/column table-header counts, header-scope
  facts, bounded structure-tree diagnostics, explicit role-map target entries,
...
- `c66ea3f` keeps Documents/Architecture **PARTIAL**: the PDF accessibility
  ...
  report JSON is version 10 and retains focused tests for no-PDF/UA-claim
  behavior.
...
- Accessibility metadata, mapped inter-word spaces, PDF table-structure
  semantics, decomposed PDF/UA blocker reports, version 10 scoped table-header
