...
*Updated 2026-07-10 from committed implementation snapshot `e5b4597`, including
the database key-ops status/migration guard, imported-document preservation
review policy, external signing workflow tool, API working-copy export matrix
tests, AMA/CMD evidence pack generator, package artifact integrity check,
release-trust metadata guard for explicit unsigned/local modes, package source
provenance validation, archive package tamper-failure tests, browser export/save
flow E2E, Docker deployment profiles, dashboard activity summaries plus active
legal-hold and sealed-not-archived archive lifecycle alerts, hardened password
verifier storage tests, imported document evidence families, signing trust
failure matrix tests, template law-reference exposure, persisted platform log
tail, guarded archive disposal evidence, web unit-test coverage thresholds,
AI/MCP assurance copy and MCP draft source provenance, bounded ASiC-S signing
support, bounded ASiC-E/CAdES manifest support, bounded tagged PDF structure,
local long-term signature evidence status, bounded law citation resolver/corpus
pin-copy UI preserving Verified/Pending status, convening evidence references
through the act/document/editor path, privacy breach-playbook and
transfer-control registers, and broader guest/minimal redaction for book, act,
and imported-document metadata. It is an implementation and test coverage
snapshot, not a legal certification, not production CMD approval, not DRE
verification promotion, and not a claim that qualified-trust production
operation, release signing/notarization/attestation, live SQLCipher
encryption/rotation/migration, legal document acceptance, or destructive GDPR
erasure is complete.*
...
- **Document import validation and persistence:** `POST /v1/documents/import/validate` accepts raw
  ...
  documents are non-canonical evidence. This does not replace preserved canonical documents, run OCR,
  convert legacy Word/image/text/bundle files, create PDF/A, or prove legal/signature validity.
...
- **MCP tool coverage:** The stdio MCP catalog includes read/write-controlled `/api/v1` tools for
  ...
  timestamp, and null model/provider placeholders; sealed or non-draft API shapes are refused
  instead of presented as drafts. The signature-bundle tool wraps the
...
- **Release trust metadata guard:** `scripts/check-release-trust.mjs` validates explicit package
  `releaseTrust` metadata and Docker signing-status metadata in the current `unsigned-dev` and
  `local-ci` modes, with a CI self-test and release/Docker workflow checks that reject production
  signing, notarization, publication, or attestation claims without matching evidence. This guards
  metadata honesty only; it does not sign, notarize, attest, publish, or prove artifact provenance.
...
- **Release and store hardening foundations:** packaging now stages `manifest.json` and `SHA256SUMS`
  ...
  operators toward an explicit backup/export-restore migration path. This is a guard and status
  surface only: it does not prove live SQLCipher encryption, decryptability, key rotation, or
  at-rest encryption completion, and it does not perform migration. Plaintext-to-encrypted
  migration, key rotation workflow, and sidecar encryption remain follow-up work.
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, and
  ...
  incident-response/transfer-control automation beyond registers, supported storage
  migration/export-restore and key rotation tooling beyond the current key-ops guard, and
...
- Documents/archive: imported-document preservation-review workflow depth, production/legal
  signed-import validation beyond the local structural/PAdES checks, full PDF/UA delivery beyond the
  bounded tagged-structure slice, richer structure trees/tagging/role maps/marked artifacts, OCR and
...
- Trust/signing depth: production/legal B-LT/B-LTA, XAdES/ASiC-XAdES, ASiC-E coverage beyond the
  ...
  AMA/CMD production approval beyond generated evidence-pack material, and external signer legal
  completion beyond invite/envelope tracking.
- Workflow breadth: legal-calendar preset depth beyond the advisory dashboard reminder, OCR,
  ...
  signer document-gated/legal completion flows, dashboard depth beyond the current
...
- AI feature layer: drafting/extraction/compare/summarize, workflow-level provenance panels beyond
  the MCP envelope/source-provenance object and settings/platform assurance panel, accepted/rejected
  human verification checkpoint state, and additional MCP transports.
- CI/release assurance: broaden browser E2E for critical workflows beyond the export/save slice,
  add or explicitly waive coverage thresholds beyond the web unit-test lane, convert compile-only
  live signature seams into controlled integration lanes where credentials/hardware exist, harden
  package provenance beyond manifest/checksum/source metadata and release-trust metadata guards,
  move Docker profiles toward production HA/ops where required, sign/notarize release packages, and
...
- **Release trust chain:** package manifests, checksums, integrity checks, and release-trust
  ...
  signing-status checks are not image signing or attestation. Release packages are not
  signed/notarized and Docker images are not signed/attested until the workflows have production
  signing identities, registry publication, and verified artifact provenance.
...
- Imported-document preservation review policy records review requirements and conservative
  original-byte/canonical-conversion status for non-canonical evidence. It does not run OCR, convert
  documents to PDF/A, create canonical records, certify legal acceptance, or validate legal effect.
...
- CI and E2E coverage improve release confidence, and web unit tests now enforce coverage
  thresholds, but the current browser suite is not exhaustive, export/save E2E is one covered slice,
  and other lanes do not have broad coverage thresholds.
- SBOMs, checksums, package manifests, source provenance metadata, release-trust metadata
  validation, Docker signing-status checks, and Docker OCI metadata are not substitutes for package
  signing, notarization, Docker signing, attestation, or reproducible-build proof; package artifact
  integrity checks and metadata guards are regression checks, not a release trust chain.
