...
*Updated 2026-07-10 from committed implementation snapshot `7267040`,
...
MCP human-review prompt. Earlier coverage text remains prior snapshot context.
...
- Documents/archive: deeper imported-document preservation-review workflow depth beyond the current
  ...
  long-term signature evidence packaging beyond the implemented sidecars and metadata projections.
...
- AI feature layer: drafting/extraction/compare/summarize, workflow-level provenance panels beyond
  the MCP envelope/source-provenance object, status resource, static human-review prompt, and
  settings/platform assurance panel, persisted accepted/rejected human verification transitions
  beyond the declared MCP checkpoint values, broader MCP prompt/resource coverage, and additional
  MCP transports.
- CI/release assurance: broaden browser E2E for critical workflows beyond the current focused
  slices, add or explicitly waive coverage thresholds beyond the web unit-test lane, convert
  ...
  metadata guards, move Docker profiles toward production HA/ops where required, sign/notarize
  release packages, and sign/attest Docker images.
...
- MCP draft human-verification states are an output envelope/checkpoint vocabulary; the MCP status
  resource is a local operability snapshot; and the MCP human-review prompt is static guidance.
  None of them are a persisted approval workflow, legal certification, provider call, hidden
  signing/trust operation, or acceptance of draft text as final minutes.
...
- Validator corpus sidecars and projected evidence metadata preserve technical external report
  context only. They are not live trust-list decisions, legal validity conclusions, qualified
  signature validation, or authority acceptance.
...
- CI and E2E coverage improve release confidence, and web unit tests now enforce coverage
  thresholds, but the current browser suite is not exhaustive, the signed-invite/export/save/browser
  slices are focused regressions, and other lanes do not have broad coverage thresholds.
