...
PAdES DSS caller validation-time attachment, PDF/UA report version 9
...
- Working tree keeps Documents/CI **PARTIAL**: PDF accessibility report JSON
  version 9 now includes bounded structure-tree diagnostics, explicit role-map
  target entries, marked-content coverage counts, writer-owned
  marked-artifact target/operator evidence via
  `writer_owned_decorative_artifacts_accounted_for`, structural-depth evidence,
  and bounded topology facts for the local tagged-PDF profile. The
...
- `c66ea3f` keeps Documents/Architecture **PARTIAL**: the PDF accessibility
  ...
  table/vote-table semantics, structure-tree diagnostics, marked-artifact
  evidence, and non-text-content accounting facts; the current deterministic
  report JSON is version 9 and retains focused tests for no-PDF/UA-claim
  behavior.
...
- **PDF/UA blocker decomposition:** the PDF accessibility report now breaks its
  ...
  structure-tree diagnostics, marked artifact target/operator evidence, and
  caller-supplied non-text content accounting. The current deterministic JSON
  report is versioned as `9` and accounts for writer-owned decorative artifacts
...
- Accessibility metadata, mapped inter-word spaces, PDF table-structure
  semantics, decomposed PDF/UA blocker reports, version 9 structural-depth,
