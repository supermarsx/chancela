...
*Updated 2026-07-10 through committed implementation snapshot `f281fb8`,
...
erasure DSR preflight evidence, and `f281fb8` expanded recent-landed checkpoint
coverage. Earlier coverage text remains prior snapshot context.
...
- **MCP tool coverage:** The stdio MCP catalog includes read/write-controlled `/api/v1` tools for
  ...
  default-off tenant gate to managers.
...
- **Recent-landed checkpoint:** `npm run test:checkpoint:recent-landed` and the GitHub Actions
  ...
  DocTimeStamp evidence tests, local PKCS#12 API signing tests, bounded retention execution tests,
  TSL XML-DSig hardening tests, web contract/dashboard/signing/i18n tests, notification-popup browser
  coverage, validator corpus sidecar validation, and desktop lockfile metadata. The static mode
  catches accidental removal of the mapped files and fixture markers without running the full
  commands.
...
- Workflow breadth: legal-calendar preset depth beyond the advisory dashboard reminder, OCR
  ...
  packages beyond current non-authoritative OCR draft metadata, written-resolution evidence capture
  beyond the advisory warning, external signer document-gated/legal completion flows, dashboard
...
- AI feature layer: drafting/extraction/compare/summarize, workflow-level provenance panels beyond
  the MCP envelope/source-provenance object, act human-review gate panel, status resource, static
...
- MCP draft human-verification states are exposed in output envelopes, and AI-origin act drafts now
  ...
  status resource is a local operability snapshot, and the MCP human-review prompt is static
  guidance.
