...
*Updated 2026-07-09 from committed implementation snapshot `09fa40f`. This replaces the stale
...
- **Platform operations and logging policy:** Settings now carries an additive `platform` section
  ...
  and `/v1/platform/services/{id}/actions/{action}` records start/stop/restart desired state with
  audit evidence. The web Settings `Operações` tab exposes the same status, log controls, and
  action buttons. This is honest desired-state/control evidence only: true stop/start/restart and
  live log tailing still require an external supervisor and structured logging sink.
- **Entity list ergonomics:** registered-entity filters now keep search/family/type in a compact
  primary row and move the rest into an accordion-style advanced filter area. Entity rows remain
  single-line with tooltips, while Type and Last Activity render shorter visible summaries to reduce
  horizontal overflow pressure.
