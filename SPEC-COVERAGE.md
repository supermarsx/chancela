...
external sync readiness is complete.*
...
- **Postgres store runtime write/read paths:** the off-by-default `postgres`
  backend now derives DDL from shared schema constants, opens an advisory-locked
  writer plus read pool, stamps schema/meta rows, replays boot state, ports the
  request-serving `Tx` write surface, and exposes runtime `Store` reads for
  ledger pages, generated/signed/imported/paper-book documents, follow-ups,
  dispatch evidence, and pending-CMD sessions. The checkpoint pins source
  markers and the ignored `DATABASE_URL` tests
  `persist_and_reload_event_roundtrips_on_postgres` and
  `runtime_reads_and_writes_roundtrip_on_postgres`; default CI still does not
  run a live Postgres database, and SQLite-file-shaped backup, restore, recovery,
  imported-book, and raw transaction paths fail closed with
  `UnsupportedOnPostgres`. This is optional runtime backend plumbing and marker
