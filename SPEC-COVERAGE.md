...
*Updated 2026-07-10 from committed implementation snapshot `f6838a8`, including
the external signing workflow tool, API working-copy export matrix tests, AMA/CMD
evidence pack generator, package artifact integrity check, release-trust metadata
guard for explicit unsigned/local modes, archive package tamper-failure tests,
browser export/save flow E2E, Docker deployment profiles, dashboard activity
summaries plus active legal-hold and sealed-not-archived archive lifecycle
alerts, hardened password verifier storage tests, imported document evidence
families, signing trust failure matrix tests, template law-reference exposure,
persisted platform log tail, guarded archive disposal evidence, web unit-test
coverage thresholds, AI/MCP assurance copy, bounded ASiC-S signing support,
bounded ASiC-E/CAdES manifest support, bounded tagged PDF structure, local
long-term signature evidence status, bounded law citation resolver/corpus
...
operation, release signing/notarization/attestation, or destructive GDPR erasure
is complete.*
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, and
  ...
  incident-response/transfer-control automation beyond registers, storage migration/recovery
  tooling, and legal-hold/disposal operator workflows beyond the dashboard advisory prompts remain
  implementable next slices before any data-lifecycle compliance claim.
...
- CI/release assurance: broaden browser E2E for critical workflows beyond the export/save slice,
  ...
  package provenance beyond manifest/checksum/integrity checks and release-trust metadata guards,
...
- SBOMs, checksums, package manifests, release-trust metadata validation, Docker signing-status
  checks, and Docker OCI metadata are not substitutes for package signing, notarization, Docker
  signing, or attestation; package artifact integrity checks and metadata guards are regression
  checks, not a release trust chain.
