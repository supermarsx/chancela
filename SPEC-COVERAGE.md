...
*Updated 2026-07-10 through committed implementation snapshot `13955be`,
...
and `13955be` API data key-rotation preflight exposure. Earlier coverage text
remains prior snapshot context. All top-level spec areas remain **PARTIAL**.
...
legal document acceptance, signed-PDF legal validity, or destructive GDPR erasure
is complete.*
...
- **MCP tool coverage:** The stdio MCP catalog includes read/write-controlled `/api/v1` tools for
  ...
  default-off tenant gate to managers. MCP also exposes a local spec-09 coverage resource and a
  compliance-pack gap-review prompt for human review of DSR, retention, archive, and signature
  evidence; both remain static/offline review aids with no provider calls or legal-validity claim.
...
- **Recent-landed checkpoint:** `npm run test:checkpoint:recent-landed` and the GitHub Actions
  ...
  PKCS#12 API signing tests, bounded retention execution tests, TSL XML-DSig hardening tests, MCP
  resource/prompt tests, web contract/dashboard/signing/i18n tests, notification-popup browser
  coverage, static imported-document review notification/export browser markers, validator corpus
  sidecar validation, and desktop lockfile metadata. The static mode catches accidental removal of
...
- **Release and store hardening foundations:** packaging now stages `manifest.json` and `SHA256SUMS`
  ...
  submitted keys from debug output. This is a guard, status, operator-plan, API preflight, and
  store-level evidence surface only: it does not prove live production SQLCipher encryption, host
  decryptability, completed UI/execution rotation operations, completed at-rest encryption, or
  plaintext migration. Plaintext-to-encrypted migration execution, production rotation workflow, and
  sidecar encryption remain follow-up work.
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, with
  ...
  migration/export-restore and UI/execution key rotation tooling beyond the current key-ops
  guard/plan plus API/store preflight and store-level rekey evidence, and
...
- Documents/archive: deeper imported-document preservation-review workflow depth beyond the current
  bounded status/note transition and guardrail checklist, production/legal signed-import validation
...
- Workflow breadth: legal-calendar preset depth beyond the advisory dashboard reminder, OCR
  ...
  packages beyond current non-authoritative OCR draft metadata and preflight evidence,
  ...
  API/UI rotation operations beyond store-level rekey evidence, ZK, HA, and mobile builds.
- AI feature layer: drafting/extraction/compare/summarize, workflow-level provenance panels beyond
  ...
  resources, static human-review/compliance prompts, and settings/platform assurance panel, broader
...
- CI/release assurance: broaden browser E2E for critical workflows beyond the current focused
  signed-invite, export/save, imported-document review/notification-export, and notification-popup
  regressions, add or explicitly waive coverage thresholds beyond the web unit-test lane, convert
...
- MCP draft human-verification states are exposed in output envelopes, and AI-origin act drafts now
  ...
  status and spec-coverage resources are local snapshots, and the MCP human-review and compliance
  prompts are static guidance.
- Local B-LT/B-LTA labels and local LTV renewal plans report technical evidence observed in the
  file, including per-signature evidence gaps and caller-supplied deadline classification where
...
- Template catalog metadata validation is regression coverage for required fields, duplicate IDs,
  family-binding drift, and law-reference anchors; it is not legal review of template wording,
  thresholds, or cited law.
...
- Database key-ops status/preflight is a secret-free configuration/build/header classification and
  startup guard with API/store key-rotation preflight/rekey evidence. It does not prove live
  production SQLCipher encryption, host decryptability, completed key-rotation execution, completed
  at-rest encryption, or a production migration path, and it does not convert plaintext SQLite
...
- Imported-document preservation review policy, UI controls, and guardrail fields record review
  requirements, reviewer metadata, and conservative original-byte/canonical-conversion decisions for
  non-canonical evidence. They do not run OCR, convert documents to PDF/A, create canonical records,
  create or validate signed artifacts, certify legal acceptance, or validate legal effect.
