...
PDF/UA report version 11 deterministic blocker-delta and scoped table-header evidence,
...
- Current `47ed33b` keeps Documents/Roles & Access/Data/CI **PARTIAL**: PDF
  accessibility report JSON version 11 adds deterministic
  `pdf_ua_blocker_delta` evidence with local basis
  `local_chancela_doc_writer_evidence_only`, cleared and remaining blocker
  lists, cleared count of 12 and remaining count of 1, and the default
  remaining `limited_tagged_structure` blocker. `pdf_ua_claimed` remains false,
  no `pdfuaid` metadata is emitted, and this is local blocker-delta evidence
  only: not PDF/UA conformance, external validator success, legal
  accessibility certification, or signed-PDF accessibility certification.
...
- Current `3fcfda3` PDF accessibility evidence projection keeps Documents,
  ...
  embed the deterministic `chancela-doc` accessibility report JSON v11 plus
...
- Working tree keeps Documents/CI **PARTIAL**: PDF accessibility report JSON
  version 11 now includes deterministic `pdf_ua_blocker_delta` evidence with
  cleared and remaining blocker lists, cleared count of 12 and remaining count
  of 1, plus scoped row/column table-header counts, header-scope facts, bounded
  structure-tree diagnostics, explicit role-map target entries,
  marked-content coverage counts, writer-owned marked-artifact target/operator
  evidence via `writer_owned_decorative_artifacts_accounted_for`,
  structural-depth evidence, and bounded topology facts for the local
  tagged-PDF profile. The
  ...
  The default remaining blocker is `limited_tagged_structure`;
...
- `c66ea3f` keeps Documents/Architecture **PARTIAL**: the PDF accessibility
  ...
  report JSON is version 11 and retains focused tests for no-PDF/UA-claim
  behavior.
...
- **PDF/UA blocker decomposition:** the PDF accessibility report now breaks its
  ...
  accounting. The current deterministic JSON report is versioned as `10` and
  accounts for writer-owned decorative artifacts
  including the header rule, explicit rules, vote-table rules, and signature
  lines while excluding page breaks; focused tests keep
...
- Accessibility metadata, mapped inter-word spaces, PDF table-structure
  semantics, decomposed PDF/UA blocker reports, version 11 deterministic
  ...
  and writer-owned marked-artifact target/operator evidence,
  bounded topology self-checks,
