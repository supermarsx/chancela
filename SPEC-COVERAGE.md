...
paper-book OCR draft to mutable draft-act creation, then `e2ff840` ASiC
...
- `e2ff840` keeps Signatures/Documents/CI **PARTIAL**: ASiC profile inspection
  now exposes structural profile-shape, per-manifest diagnostics, per-signature
  ...
  signatures, and manifest digest mismatches; tests also prove these diagnostics
  do not relax extraction/validation refusal. This is structural diagnostics
  only, not XAdES generation or validation, CAdES trust validation, LTV evidence,
  legal validity, qualified status, production ASiC compliance, or authority
  approval.
...
- `c66ea3f` keeps Documents/Architecture **PARTIAL**: the PDF accessibility
  ...
  facts, emits deterministic report JSON version 4, and retains focused tests
  for no-PDF/UA-claim behavior. This is technical blocker explanation only, not
...
- **User and signatory email capture:** onboarding first-user creation, Settings
  ...
  and send them through existing backend contracts. Book opening/closing
  signatory requirements remain string-only until a structured backend contract
  exists. This is field capture only, not email verification, identity proof, or
...
- **Platform service controls and log thresholds:** platform APIs now expose
  ...
  true per-service overrides. This is not process supervision or stdout/MCP log
  tailing.
...
- **Dashboard summary caps and polish:** the dashboard current-work tab now shows
  ...
  notice semantics.
- **ASiC structural diagnostics:** ASiC profile inspection now reports structural
  profile shape, manifest/signature diagnostics, and stable blocker IDs for
  unsupported or inconsistent ASiC member shapes such as missing referenced
  signatures, unreferenced signatures, and manifest digest mismatches. Focused
  tests prove the diagnostic report does not relax bounded ASiC-E extraction
  refusal. This is structural diagnostics only, not XAdES validation, CAdES trust
  validation, LTV evidence, legal validity, qualified status, or authority
  approval.
- **Accepted paper-book OCR draft to mutable act draft:** accepted OCR draft text
  can now be copied into a new mutable `Draft` act through
  `POST /v1/books/paper-import/{id}/ocr-drafts/{draft_id}/canonical-draft`.
  The API requires an accepted persisted draft with extracted text, an open
  target book, and `act.draft` permission; it refuses non-accepted, closed-book,
  and digest-only cases without act or ledger mutation. The ledger event records
  only metadata/digest/presence flags and explicitly keeps raw OCR text out.
  The book detail UI exposes a "Criar rascunho de ata" action only after
  acceptance and reports that no canonical document, PDF/A, signature, seal, or
  legal validity was created. This is a drafting aid only, not authoritative OCR
  text, canonical minutes conversion, document generation, signing, sealing, OCR
  accuracy certification, legal acceptance, or legal validity.
...
- **PDF/UA blocker decomposition:** the PDF accessibility report now breaks its
  ...
  JSON report is versioned as `4`, and focused tests keep `pdf_ua_claimed` false
  plus the absence of PDF/UA identification metadata. This is blocker
...
- **Platform operations and logging policy:** Settings now carries an additive `platform` section
  ...
  validation. API-owned structured entries are recorded only when the emitted level passes the
  ...
  fallback remains. Platform status reads and control requests add structured API log entries only
  when those thresholds allow them. The web Settings
...
- **Historical paper-book validation and preservation:** `POST /v1/books/paper-import/validate`
  ...
  false canonical-act/document/signature/legal-validity flags. Broader OCR execution/review beyond
  the operator-configured local auxiliary draft path, reviewed canonical conversion execution, and
  legal acceptance remain follow-up work.
...
- **Recent-landed checkpoint:** `npm run test:checkpoint:recent-landed` and the GitHub Actions
  ...
  identifier UI markers, external-validator metadata panel/client/i18n markers, PDF table-structure
  ...
  signatory email markers, compact data-cleanup row markers, Settings trust-source
  provider/i18n markers, hardening-plan head markers,
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, with
  ...
  non-destructive evidence only; deeper backend per-table payload statistics, actual physical deletion, broader retention/disposal policy
...
- Documents/archive: deeper imported-document preservation-review workflow depth beyond the current
  ...
  operations and reviewed canonical/legal conversion beyond
  the operator-configured local auxiliary OCR draft path plus accepted-draft
  mutable drafting aid and preflight evidence for preserved legacy DOC and
  ...
  plans, caller-supplied local archive timestamp append, and technical metadata projections.
...
- Workflow breadth: legal-calendar preset depth beyond the advisory dashboard reminder, broader OCR
  ...
  accepted-draft mutable drafting aid, focused browser workflow regression, and
  preflight evidence, written-resolution legal/evidentiary
  ...
  structured book opening/closing signatory email contracts beyond the current string-only lists,
...
- Local B-LT/B-LTA labels, caller-supplied local archive timestamp append, and local LTV renewal
  plans report or mutate technical evidence observed in the file, including multi-signature DSS/VRI
  coverage, per-signature evidence gaps, local `/DocTimeStamp` presence, and caller-supplied
  deadline classification where configured; they are not production or legal long-term-validation
  claims, legally derived renewal-deadline decisions, provider-driven renewal, production B-LTA, or
...
- ASiC structural diagnostics expose member-shape classifications, manifest/signature diagnostics,
  and blocker IDs so operators/tests can understand why a package is outside the bounded slice.
  They do not validate XAdES, establish CAdES trust, fetch or prove LTV evidence, certify broad
  ASiC profile compliance, or create legal/qualified-signature validity.
...
- Paper-book local OCR run, OCR draft review metadata, accepted-draft-to-act
  drafting, UI, and focused browser workflow coverage record bounded command
  status and non-authoritative review/drafting evidence with explicit false
  canonical/signature/legal flags. The accepted-draft path copies OCR text into
  mutable Draft deliberations as a drafting aid and keeps raw OCR text out of
  ledger events; it does not treat that text as authoritative, create canonical
...
- Paper-book canonical-conversion preflight evidence classifies whether an operator-supplied
  ...
  accepted-draft-to-act path is still only a mutable drafting aid; neither path
  executes authoritative OCR, converts paper/legacy evidence into canonical
  minutes, creates canonical documents, signs artifacts, certifies legal
  acceptance, or claims legal validity.
...
- Accessibility metadata, mapped inter-word spaces, PDF table-structure semantics,
  decomposed PDF/UA blocker reports, bounded
  tagged structure, `DisplayDocTitle`, `/Tabs /S`, XMP consistency checks, and structural
  self-checks are not PDF/UA delivery; the writer still keeps `pdf_ua_claimed: false` and reports
  local blocker facts rather than certification.
...
- The persisted platform log tail covers threshold-filtered API-owned structured events only and is
  bounded to 512 entries; it is not stdout/stderr capture, MCP child-process logging, complete
  historical logging, supervisor lifecycle evidence, or a general observability sink.
