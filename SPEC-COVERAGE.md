...
*Updated 2026-07-15 from current implementation snapshot `a7125b323f6c5e4811fe82628dd7e789f03664f1`,
...
condominium annual local advisory due-date coverage and `a7125b3`
all-filtered archive export coverage.
...
- Current `a7125b3` keeps Data/Archive/UX/CI **PARTIAL**: Arquivo export now has
  an explicit `export_scope`, preserving the default bounded current-page
  export while adding opt-in `all_filtered` export. The API still rejects
  `before_seq` for archive documents; with `export_scope=all_filtered` it walks
  matching filtered newest-first records server-side in 250-record internal
  chunks, reports `internal_batch_limit=250` and `page_limit=null` for the JSON
  export metadata, and keeps `has_more=false` / `next_cursor=null` in the final
  export response. The web UI exposes "current page" and "all filtered" export
  scopes, clears the UI page limit for all-filtered export, preserves the
  `pdfa`, `txt`, `json`, `csv`, and `html` format set, and the route-stubbed
  `/arquivo` proof verifies all-filtered export does not load older records into
  the table. Focused evidence passed: `cargo test -p chancela-api arquivo
  --locked`, `cargo test -p chancela-api --lib ledger_archive_document
  --locked`, `npm run test --workspace apps/web -- LedgerPage.test.tsx`,
  `npm run test --workspace apps/web -- client.test.ts i18n.test.ts`,
  `npm run build --workspace apps/web`, `npx playwright test
  e2e/ledger-archive-boundedness.spec.ts --project=chromium`, and `git diff
  --check`. This is filtered ledger archive export coverage only: the
  all-filtered path currently assembles the final response in memory and has no
  record-count cap. It does not add DGLAB/legal archive certification, expanded
  PDF/A, signature, custody, filing/legal-acceptance, ledger-mutation, or full
  Data/Archive/UX/CI completion claims.
