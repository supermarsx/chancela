...
*Updated 2026-07-13 from committed implementation snapshot `7fcf5ef5f1c2fbd5b9eb26d6aac5c1240144a365`,
with committed evidence refreshes for the manual-signature
original-reference sealing path and web accessibility/focus guards, while preserving the prior
...
- Current worktree keeps Signatures/Documents/CI **PARTIAL**: the
  ...
  `RemoteSigningSource` initiate/confirm one-digest flow. Each document still
  opens and confirms its own remote session/activation, and the seam is
  core-only: no API route, no web UI. This is not provider-certified remote
...
- **Core repeated remote-session orchestration seam:** `chancela-signing`
  ...
  session/activation. There is no API route and no web UI. This is not
...
- The core repeated remote-session helper orchestrates repeated per-document
  remote sessions over the existing `RemoteSigningSource` initiate/confirm
  one-digest flow. Each document still opens and confirms its own remote
  session/activation. This is core-only: no API route, no web UI; it is not
  provider-certified remote batch, not single OTP/PIN/SAD authorizing multiple
  documents, not CMD multiple-sign, not CSC/QTSP multi-hash/SAD batch, and not
  SCAP/legal-capacity proof.
