...
Convening dispatch matrix note: current `caae1bf` adds WFL/UX/API evidence for
local workflow evidence capture through the existing act convening dispatch
endpoint only. It keeps every top-level row **PARTIAL** because it records
metadata for already-present `act.convening` recipients and does not create
recipients, send email/SMS, prove delivery, compute legal sufficiency, complete
