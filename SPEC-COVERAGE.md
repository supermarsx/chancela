...
*Updated 2026-07-10 through committed implementation snapshot `270400f`,
...
review checkpoint pinning. Earlier coverage text remains prior snapshot context.
...
- **Release and store hardening foundations:** packaging now stages `manifest.json` and `SHA256SUMS`
  ...
  with post-rekey integrity checking. This is a guard, status, operator-plan, and store-level
  evidence surface only: it does not prove live production SQLCipher encryption, host
  decryptability, API/UI rotation operations, completed at-rest encryption, or plaintext migration.
  Plaintext-to-encrypted migration execution, production rotation workflow, and sidecar encryption
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, with
  ...
  migration/export-restore and API/UI key rotation tooling beyond the current key-ops guard/plan
  plus store-level rekey evidence, and
...
- Database key-ops status/preflight is a secret-free configuration/build/header classification and
  startup guard with store-level key-rotation preflight/rekey evidence. It does not prove live
  production SQLCipher encryption, host decryptability, API/UI key rotation, completed at-rest
  encryption, or a production migration path, and it does not convert plaintext SQLite stores into
  encrypted stores.
