...
- Documents/archive: deeper imported-document preservation-review workflow depth beyond the current
  ...
  the operator-configured local auxiliary OCR draft path plus preflight evidence for preserved
  legacy DOC and historical paper-book evidence, official DGLAB
...
- Workflow breadth: legal-calendar preset depth beyond the advisory dashboard reminder, broader OCR
  ...
  operator-configured local OCR run, non-authoritative OCR draft metadata/review UI, focused
  browser workflow regression, and preflight evidence, written-resolution legal/evidentiary
...
- Paper-book local OCR run, OCR draft review metadata, UI, and focused browser
  workflow coverage record bounded command status and non-authoritative review
  drafts with explicit false canonical/signature/legal flags. They do not store
  authoritative text, create canonical minutes, create canonical documents, sign
  anything, certify OCR accuracy, or accept historical scans as legally converted
  digital records.
