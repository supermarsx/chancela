...
*Updated 2026-07-15 from current implementation snapshot `c2194d90ebf970ff51ecfc308b5805059c926492`,
with committed evidence refreshes for the recently landed convening recipient
editor before local dispatch evidence, route-stubbed convening dispatch browser
proof, convening dispatch evidence capture UI slice,
...
`c2194d9` local convening recipient editing before dispatch evidence stamping.
...
- Current `c2194d9` keeps Workflows/Legal/Compliance/UX/API/CI **PARTIAL**:
  the Ata editor now exposes a convening recipient row editor for recipient
  name, contact/reference, channel, and dispatched date, then persists those
  rows through the existing `updateAct` / `UpdateActBody.convening` path. Blank
  names are filtered out of the saved convening payload, and the
  contact/reference value maps to the existing `reference` field because no
  separate contact field exists. Local dispatch evidence remains gated on
  persisted act state: UI-created recipients must be saved first, and the
  dispatch evidence action stays disabled until recipient names exist in the
  persisted act. Focused evidence passed: `npm run test --workspace apps/web --
  AtaEditorStructured.test.tsx` (27 tests), `npm run test:browser --workspace
  apps/web -- e2e/convening-dispatch-evidence.spec.ts` (1 Chromium test), `npm
  run build --workspace apps/web`, `git diff --check`, and `git diff --check
  HEAD~1..HEAD`. This is local workflow metadata and evidence-capture support
  only: copy remains local evidence only, with no email/SMS/provider delivery,
  legal sufficiency, compliance completion, registry/DRE acceptance, provider
...
- Current `9ddced8` keeps Data/Architecture/CI **PARTIAL**: the
  ...
  for settings/users/roles/sidecars, live DB validation, migration
...
Convening dispatch matrix note: current `c2194d9` adds WFL/UX/API evidence for
local convening recipient metadata editing before local dispatch evidence
capture. The earlier `caae1bf` evidence still owns dispatch stamping through
the existing act convening dispatch endpoint. It keeps every top-level row
**PARTIAL** because rows are local workflow metadata, blank-name rows are not
saved, contact/reference is the existing `reference` field, UI-created
recipients must be saved before evidence stamping, and the slice does not send
email/SMS, prove delivery, compute legal sufficiency, complete
workflow/legal/compliance status, or claim registry/DRE/provider acceptance.
...
- **Postgres store runtime write/read and logical recovery paths:** the
  ...
  legal/DR certification, external sync readiness, or a replacement for the
  SQLite/SQLCipher validation lanes.
