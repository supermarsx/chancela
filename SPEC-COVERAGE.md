...
*Updated 2026-07-10 through committed implementation snapshot `76f335a48075856024980eac8eaa2ad7398a7d1a`,
...
`76f335a` CLI database-encryption key-env parity. Earlier coverage text remains prior snapshot context. All top-level spec areas remain **PARTIAL**.
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, with
  ...
  guard/plan plus web/API/CLI/store preflight/key-env support and store-level rekey evidence, and
...
- Workflow breadth: legal-calendar preset depth beyond the advisory dashboard reminder, OCR
  ...
  API/UI/CLI rotation operations beyond current key-env/preflight support and store-level rekey evidence, ZK, HA, and mobile builds.
...
- Database key-ops status/preflight is a secret-free configuration/build/header classification and
  startup guard with web/API/CLI/store key-env/preflight/rekey evidence. It does not prove live
