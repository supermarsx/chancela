...
*Updated 2026-07-10 from committed implementation snapshot `236e167`, including
the platform log, entity-list ergonomics, data-storage status, local PDF
signature-validation, PDF validator UI, template-filter, and trust-provider
settings slices. It is an implementation coverage snapshot, not a legal
...
|---|---|---|---|---|
| spec/01 Product Scope (SCP) | PARTIAL | One Rust core still drives server, web, Docker, and Tauri desktop. Durable mode exists through `CHANCELA_DATA_DIR`; in-memory mode is explicit on `/health`, and Settings now exposes read-only storage mode/data-folder/usage telemetry from `/v1/data/status`. | Mobile companion remains deferred; edition packaging still needs signed/notarized publication hardening. | None specific. |
...
- **Data storage visibility:** `GET /v1/data/status` reports the current persistence mode,
  configured data folder, directory existence/type, read/create/write/delete permission probes,
  durable SQLite open state, schema/ledger counters, and filesystem plus SQLite logical usage
  breakdowns. Settings -> Dados renders the same status with refresh, copy-path, scan-error, and
  browser-safe open-folder-disabled states. This is read-only diagnostics; it does not add durable
  log retention, SQLCipher production enablement, or storage migration tooling.
...
- **Configurable trust source settings:** Settings now serializes and validates additive
  `signing.tsl_sources` and `signing.tsa_providers` arrays while preserving the legacy
  `signing.tsl_url` and `signing.tsa_url` fields. Validation covers stable IDs, duplicate IDs,
  URL/path presence, URL shape, digest/policy formats, refresh cadence, timeouts, byte caps, and the
  single enabled default TSA invariant. This is configuration plumbing only; the trust resolver and
  signing flows still use their existing runtime paths until a later integration slice consumes the
  arrays.
...
- **TSL/TSA:** the catalog can parse cached or fixture XML and settings can now hold multiple TSL
  sources and TSA providers, but live trust operation still needs those settings wired into the
  resolver/signing flows, valid live source material, and policy review. TSA diagnostics use an
  offline fixture probe unless a signing flow explicitly requests a timestamp. The bundled fixture
  is advisory and must not be treated as legal trust completion.
