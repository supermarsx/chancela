...
*Updated 2026-07-10 from committed implementation snapshot `5d56b86`, including
the external signing workflow tool, API working-copy export matrix tests, AMA/CMD
evidence pack generator, package artifact integrity check, archive package
tamper-failure tests, browser export/save flow E2E, Docker deployment profiles,
dashboard activity summaries, hardened password verifier storage tests, imported
document evidence families, signing trust failure matrix tests, template
law-reference exposure, persisted platform log tail, guarded archive disposal
evidence, web unit-test coverage thresholds, AI/MCP assurance copy, bounded
ASiC-S signing support, bounded tagged PDF structure, local long-term
signature evidence status, bounded law citation resolver/corpus pin-copy UI
preserving Verified/Pending status, convening evidence references through the
act/document/editor path, privacy breach-playbook and transfer-control
registers, and broader guest/minimal redaction for book, act, and
imported-document metadata. It is an implementation and test coverage snapshot,
not a legal certification, not production CMD approval, not DRE verification
promotion, and not a claim that qualified-trust production operation or
...
- Data lifecycle/storage: the cleanup endpoint covers crash reports and retained exports only, and
  ...
  tooling, and legal-hold-aware operator workflows remain implementable next slices before any
  data-lifecycle compliance claim.
...
- Trust/signing depth: production/legal B-LT/B-LTA, XAdES, ASiC-E/XAdES/multi-payload/manifests,
  ...
  collection for the corpus, AMA/CMD production approval beyond generated evidence-pack material,
  and external signer legal completion beyond invite/envelope tracking.
- Workflow breadth: legal-calendar preset depth beyond the advisory dashboard reminder, OCR,
  ...
  signer document-gated/legal completion flows, dashboard depth beyond compact activity summaries,
  groups/tenancy,
...
- CI/release assurance: broaden browser E2E for critical workflows beyond the export/save slice,
  ...
  package provenance beyond manifest/checksum/integrity checks, move Docker profiles toward
  production HA/ops where required, sign/notarize release packages, and sign/attest Docker images.
...
- **Qualified-signature requirements:** CMD/legal-signature copy must stay explicit that there is no
  ...
  requirements.
- **Release trust chain:** package manifests, checksums, and integrity checks are not signing or
  notarization; Docker profiles and OCI metadata are not image signing or attestation. Release
  packages are not signed/notarized and Docker images are not signed/attested until the workflows
  have production signing identities, registry publication, and verified artifact provenance.
...
- SBOMs, checksums, package manifests, and Docker OCI metadata are not substitutes for package
  signing, notarization, Docker signing, or attestation; package artifact integrity checks are
  regression checks, not a release trust chain.
