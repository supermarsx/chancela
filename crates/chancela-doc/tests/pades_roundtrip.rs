...
// --- tg4: the rest of the signature-annotation profile ------------------------------------------
//
// The signed profile's *entry* rules were pinned above; the annotation rules underneath them were
// mostly never observed to fire — `selfcheck/annots.rs` executed 53 % of its functions, and almost
// every unexecuted one is a `return Err(...)` on the signature surface. Each mutant below is
// equal-length, for the reason stated at the top of this section, and each asserts the specific
// rule's own diagnostic rather than merely that verification failed.

/// Apply `mutate` to a freshly signed document and return the signed-profile error.
fn signed_mutant(serial: u8, mutate: impl FnOnce(&mut Vec<u8>)) -> String {
    let (_, mut signed) = signed_fixture(serial);
    mutate(&mut signed);
    selfcheck::verify_signed(&signed)
        .expect_err("the mutated signature revision must be refused")
        .to_string()
}

/// The same over a visibly-sealed document, whose `/AP /N` stream is where a signed file can
/// acquire fonts and operators that never passed through the writer.
fn sealed_mutant(serial: u8, mutate: impl FnOnce(&mut Vec<u8>)) -> String {
    let base = pdfa::write(&ata_fixture()).expect("write base");
    let mut signed = sign_with_text_seal(&base, "Amélia Marques Gonçalves", serial);
    mutate(&mut signed);
    selfcheck::verify_signed(&signed)
        .expect_err("the mutated seal appearance must be refused")
        .to_string()
}
...
/// `/SigFlags` bit 1 (SignaturesExist) is what tells a reader the `/AcroForm` holds signatures at
/// all. Cleared, a conformant reader may never look for them — the signature is present in the
/// bytes and invisible to the tool that would validate it.
#[test]
fn a_signed_acroform_must_declare_that_signatures_exist() {
    let err = signed_mutant(30, |signed| {
        replace_once(signed, b"/SigFlags 3", b"/SigFlags 0")
    });
    assert_diagnostic(&err, "SignaturesExist");
}

/// A widget that is not an `/FT /Sig` field is not a signature field: whatever `/V` it carries is
/// not a signature value, and nothing would validate it.
#[test]
fn a_widget_that_is_not_a_signature_field_is_rejected() {
    let err = signed_mutant(31, |signed| {
        let at = signed
            .windows(8)
            .rposition(|w| w == b"/FT /Sig")
            .expect("the widget /FT");
        replace_once(&mut signed[at..], b"/FT /Sig", b"/FT /Six");
    });
    assert_diagnostic(&err, "/FT /Sig signature field");
}

/// PDF/A-2 §6.3.1 prohibits `/A` and `/AA` on annotations. An action on the signature widget would
/// make the file's behaviour depend on the reader — exactly what an archival format exists to
/// remove. The field's `/T` partial name is the same length and is not itself checked, so the
/// mutant adds an action without disturbing any rule that runs before this one.
#[test]
fn an_action_on_the_signature_widget_is_rejected() {
    let err = signed_mutant(32, |signed| {
        let at = signed
            .windows(3)
            .rposition(|w| w == b"/T ")
            .expect("the widget /T partial field name");
        replace_once(&mut signed[at..], b"/T ", b"/A ");
    });
    assert_diagnostic(&err, "/A or /AA action");
}

/// Print must be set and Hidden/Invisible/NoView must be clear. `/F 133` is Print + Locked +
/// **Invisible**: the signature would print but not display, so a reader sees an unsigned-looking
/// page for a signed file.
#[test]
fn an_invisible_signature_widget_is_rejected() {
    let err = signed_mutant(33, |signed| replace_once(signed, b"/F 132", b"/F 133"));
    assert_diagnostic(&err, "Invisible");
}

/// The `/AcroForm /Fields` array and the widgets actually attached to pages must be the same set.
/// A widget missing from `/Fields` is a signature a form-aware reader never enumerates.
#[test]
fn a_widget_absent_from_the_acroform_fields_array_is_rejected() {
    let err = signed_mutant(34, |signed| {
        // Repoint the /Fields entry's object number at an object that is not the widget. The
        // array stays a well-formed one-element array of references, so only the set comparison
        // between /Fields and the attached widgets can catch it.
        let at = signed
            .windows(9)
            .rposition(|w| w == b"/Fields [")
            .expect("the /AcroForm /Fields array")
            + b"/Fields [".len();
        bump_decimal_digit(signed, at);
    });
    assert_diagnostic(&err, "not listed in /AcroForm /Fields");
}

/// The seal's appearance is a form XObject. Anything else is a construct the signer does not emit,
/// and the closed profile the page content is held to would not apply to it.
#[test]
fn a_seal_appearance_that_is_not_a_form_xobject_is_rejected() {
    let err = sealed_mutant(35, |signed| {
        let at = signed
            .windows(14)
            .rposition(|w| w == b"/Subtype /Form")
            .expect("the seal appearance /Subtype");
        replace_once(&mut signed[at..], b"/Subtype /Form", b"/Subtype /Forl");
    });
    assert_diagnostic(&err, "/Subtype /Form XObject");
}

/// Without a `/BBox` the appearance has no declared extent, so where the seal is drawn is left to
/// the reader.
#[test]
fn a_seal_appearance_without_a_bounding_box_is_rejected() {
    let err = sealed_mutant(36, |signed| {
        let at = signed
            .windows(5)
            .rposition(|w| w == b"/BBox")
            .expect("the seal appearance /BBox");
        replace_once(&mut signed[at..], b"/BBox", b"/BBoy");
    });
    assert_diagnostic(&err, "form XObject has no /BBox");
}

/// A seal font with no `/ToUnicode` extracts as nothing: the signer's name would render and be
/// uncopyable, which is precisely the "u" in PDF/A-2U that this pipeline claims.
#[test]
fn a_seal_font_without_a_tounicode_cmap_is_rejected() {
    let err = sealed_mutant(37, |signed| {
        let at = signed
            .windows(10)
            .rposition(|w| w == b"/ToUnicode")
            .expect("the seal font /ToUnicode");
        replace_once(&mut signed[at..], b"/ToUnicode", b"/ToUnicodf");
    });
    assert_diagnostic(&err, "has no /ToUnicode CMap");
}

/// A signature field with no `/V` carries no signature value: the widget is drawn, the `/AcroForm`
/// lists it, and there is nothing to validate. `/X` is the same length and is not a key any rule
/// reads, so the mutant removes exactly one thing.
#[test]
fn a_signature_field_with_no_signature_value_is_rejected() {
    let err = signed_mutant(38, |signed| {
        let at = signed
            .windows(8)
            .rposition(|w| w == b"/FT /Sig")
            .expect("the widget /FT");
        let v = find(&signed[at..], b"/V ").expect("the widget /V") + at;
        replace_once(&mut signed[v..], b"/V ", b"/X ");
    });
    assert_diagnostic(&err, "no /V signature value");
}

/// PDF/A requires the Print flag, which means `/F` must exist to carry it. A widget with no `/F`
/// at all defaults to every flag clear — including Print — so the signature would not appear on a
/// printed copy of a document whose whole purpose is to be printable evidence.
#[test]
fn a_signature_widget_with_no_flags_at_all_is_rejected() {
    let err = signed_mutant(39, |signed| replace_once(signed, b"/F 132", b"/G 132"));
    assert_diagnostic(&err, "no /F flags integer");
}
