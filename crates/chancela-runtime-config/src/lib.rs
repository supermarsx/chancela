...
use chancela_store::{Store, StoreBackendSelection, StoreError, StoreKeyOpsPlan, StoreOpenOptions};
...
/// Settings slice consumed by both the API's embedded projection and the standalone projector.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
pub struct SearchProjectionRuntimeSettings {
    pub search: chancela_search::SearchSettings,
    pub workflow: SearchProjectionWorkflowSettings,
    pub data_management: SearchProjectionDataManagementSettings,
}

#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
pub struct SearchProjectionWorkflowSettings {
    pub reminders: chancela_action_center::WorkflowReminderSettings,
}

#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
pub struct SearchProjectionDataManagementSettings {
    pub backup_recovery: chancela_action_center::BackupRecoveryPolicySettings,
}

/// Open the query-only projector store and resolve its shared runtime settings contract.
pub fn search_projector_store_and_settings_from_env()
-> Result<(Store, SearchProjectionRuntimeSettings), AppStateInitError> {
    let store = search_projector_store_from_env()?;
    let data_dir = required_search_projector_data_dir()?;
    let settings = search_projector_settings_with_store(&store, &data_dir)?;
    Ok((store, settings))
}

pub fn search_projector_store_from_env() -> Result<Store, AppStateInitError> {
    let encryption = DatabaseEncryptionConfig::from_env()?;
    let data_dir = required_search_projector_data_dir()?;
    open_search_projector_store(&data_dir, &encryption)
}

fn required_search_projector_data_dir() -> Result<PathBuf, AppStateInitError> {
    resolve_data_dir().ok_or_else(|| {
...
/// Reload the exact settings slice used by a projector candidate.
pub fn search_projector_settings_with_store(
    store: &Store,
    data_dir: &Path,
) -> Result<SearchProjectionRuntimeSettings, AppStateInitError> {
    let loaded = if store.cluster_election_enabled() {
        match store
            ...
            None => SearchProjectionRuntimeSettings::default(),
        }
    } else {
        load_search_projector_settings_file(&data_dir.join(SETTINGS_FILE)).map_err(|source| {
...
fn load_search_projector_settings_file(
    path: &Path,
) -> Result<SearchProjectionRuntimeSettings, StoreError> {
    let journal_path = path.with_file_name(SETTINGS_AUDIT_JOURNAL_FILE);
    ...
    let bytes = match std::fs::read(path) {
        Ok(bytes) => bytes,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            return Ok(SearchProjectionRuntimeSettings::default());
        }
        Err(error) => return Err(StoreError::Io(error)),
    };
...
/// Environment variable selecting the durable Chancela data directory.
pub const DATA_DIR_ENV: &str = "CHANCELA_DATA_DIR";

/// Resolve the data directory used by every Chancela runtime.
///
/// An explicit non-blank environment value wins. Otherwise the current directory and its
/// ancestors are searched for the conventional `chancela-data` directory.
...
/// Open the dedicated query-only store used by the external search projector.
pub fn open_search_projector_store(
    data_dir: &Path,
    encryption: &DatabaseEncryptionConfig,
) -> Result<Store, AppStateInitError> {
    if encryption.is_configured() {
        let key_ops = Store::key_ops_status(data_dir, &encryption.store_open_options()).map_err(
            |source| AppStateInitError::StoreOpen {
                data_dir: data_dir.to_path_buf(),
                source,
            },
        )?;
    ...
    let backend = resolve_search_projector_backend_selection(data_dir, encryption)?;
...
///
/// The default is plaintext SQLite. When a key is present, the caller opens the durable store
/// through `StoreOpenOptions` and fails closed unless this crate was built with the
/// `sqlcipher` feature.
...
impl fmt::Display for AppStateInitError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::CorsConfiguration(message) => {
                write!(
                    f,
                    "invalid CHANCELA_CORS_ALLOWED_ORIGINS configuration: {message}"
                )
            }
...
pub fn resolve_search_projector_backend_selection(
    data_dir: &Path,
    encryption: &DatabaseEncryptionConfig,
) -> Result<ResolvedBackend, AppStateInitError> {
    match backend_kind_from_env()? {
...
mod tests {
    ...
    fn no_key_config_preserves_plaintext_store_startup() {
        ...
        let dir = TempDir::new();
        let store = match Store::open_with_options(dir.path(), config.store_open_options()) {
            Ok(store) => store,
    ...
    fn configured_key_fails_closed_without_sqlcipher_feature() {
        ...
        let err = match open_search_projector_store(dir.path(), &config) {
