...
const GENERATED_PFX_PATH_ENV: &str = "CHANCELA_PERF_GENERATED_PKCS12_PATH";
const GENERATED_PFX_PASSWORD_ENV: &str = "CHANCELA_PERF_PKCS12_PASSPHRASE";
const GENERATED_PFX_FRIENDLY_NAME: &str = "Chancela ephemeral performance identity";
...
#[test]
#[ignore = "requires a disposable PFX generated by scripts/perf/generate-test-pkcs12.sh"]
fn generated_performance_pkcs12_loads_and_signs_detached_cades() {
    let pfx_path = std::env::var_os(GENERATED_PFX_PATH_ENV)
        .expect("generated performance PFX path environment variable must be set");
    let password = Zeroizing::new(
        std::env::var(GENERATED_PFX_PASSWORD_ENV)
            .expect("generated performance PFX password environment variable must be set"),
    );
    let pfx = fs::read(pfx_path).expect("read generated performance PFX");
    let source = Pkcs12SigningSource::from_der_with_selector(
        &pfx,
        &password,
        &Pkcs12IdentitySelector::by_friendly_name(GENERATED_PFX_FRIENDLY_NAME),
    )
    .expect("load generated performance PFX through the runtime parser");

    assert_eq!(
        source.identity().friendly_name.as_deref(),
        Some(GENERATED_PFX_FRIENDLY_NAME)
    );
    assert_eq!(source.evidentiary_level(), EvidentiaryLevel::Advanced);
    assert_cades_round_trip(&source);
}
