...
/// SHA-256 of the canonical, concrete document-layout snapshot persisted with a generated
/// document. This is intentionally a separate binding from `template_spec_digest`: the latter
/// continues to identify only the producing template revision, while this digest identifies the
/// fully-resolved renderer policy that actually produced the PDF bytes.
pub fn document_layout_digest_of(canonical_layout_json: &str) -> String {
    let digest: [u8; 32] = Sha256::digest(canonical_layout_json.as_bytes()).into();
    crate::hex::hex(&digest)
}

/// Resolve the one concrete renderer policy for a template-derived document.
///
/// Precedence is fixed by the core contract and applied leaf-by-leaf:
/// instance → template → entity → book. Resolution happens before rendering or page counting, so
/// the generated bytes, capacity reservation and persisted snapshot all describe the same policy.
pub(crate) fn resolve_effective_document_layout(
    instance: &DocumentLayoutPolicy,
    spec: &TemplateSpec,
    entity: Option<&Entity>,
    book: Option<&Book>,
) -> Result<DocumentLayoutPolicy, ApiError> {
    chancela_core::resolve_document_layout(
        instance,
        spec.document_layout_override.as_ref(),
        entity.and_then(|value| value.document_layout_override.as_ref()),
        book.and_then(|value| value.document_layout_override.as_ref()),
    )
    .map(|resolved| resolved.policy)
    .map_err(|error| ApiError::Unprocessable(format!("invalid effective document layout: {error}")))
}

/// Snapshot the current instance default without retaining the settings lock across generation or
/// any entity/book/ledger lock.
pub(crate) async fn current_instance_document_layout(state: &AppState) -> DocumentLayoutPolicy {
    state
        .settings
        .read()
        .await
...
/// Parse and validate the concrete layout frozen on a stored document.
///
...
pub(crate) fn stored_document_layout(
    ...
    let Some(json) = document.document_layout_json.as_deref() else {
        return Ok(None);
    };
    let policy: DocumentLayoutPolicy = serde_json::from_str(json).map_err(|error| {
        ApiError::Internal(format!(
            "stored document {} has malformed document layout: {error}",
            document.id
        ))
    })?;
    policy.validate().map_err(|error| {
        ApiError::Internal(format!(
            "stored document {} has invalid document layout: {error}",
            document.id
        ))
    })?;
    Ok(Some(policy))
...
fn generate(
    ...
    let mut model = chancela_templates::render_with_body(spec, ctx, body)
        .map_err(|e| ApiError::Internal(format!("template render failed: {e}")))?;
    model.document_layout = document_layout.clone();
    ...
    // Serialize the concrete policy directly rather than the DocumentModel: product defaults are
    // intentionally omitted from the model's backward-compatible wire form, but every new stored
    // document must carry an explicit frozen renderer policy.
    let document_layout_json = serde_json::to_string(&model.document_layout)
        .map_err(|e| ApiError::Internal(format!("document layout serialization failed: {e}")))?;
    let document_layout_digest = document_layout_digest_of(&document_layout_json);
    ...
    let stored = StoredDocument {
        ...
        document_layout_json: Some(document_layout_json),
    };
    let event_payload = json!({
        ...
        // Separate from the template digest: this binds the concrete renderer policy after all
        // instance/template/entity/book inheritance has been resolved.
        "document_layout_digest": document_layout_digest,
    });
    Ok(Generated {
        stored,
        event_payload,
...
pub(crate) fn generate_for_act(
    ...
    let document_layout =
        resolve_effective_document_layout(instance_layout, spec, Some(entity), Some(book))?;
    ensure_template_can_carry_body(act, spec)?;
    let ctx = act_ctx(act, entity)?;
    let body_blocks = freeze_act_body(act, entity)?
        .map(|(_, blocks)| blocks)
        .unwrap_or_default();
    let mut made = generate(
        spec,
        &ctx,
        act.id,
...
/// Re-render an encerramento against an already-frozen concrete layout. This is the stale-fact
/// verifier's entry point: entity/book content facts may move and therefore must be re-derived, but
/// mutable layout layers must never alter the comparison after signatories signed the snapshot.
pub(crate) fn generate_encerramento_snapshot_with_layout(
    termo: &TermoInstrument,
    book: &Book,
    entity: &Entity,
    ata_count: u64,
...
pub async fn generate_document(
    ...
    let made = generate_for_act_template(act, book, entity, &q.template_id, &instance_layout)?;
...
pub(crate) async fn publish_generated_document_read_model(
    ...
    // Durable states serve generated bytes from the store. Retaining another full PDF/A in this
    // process would make the writer's heap grow linearly with the document corpus (and differ from
    // the empty-on-restart read model), while every durable read path already falls back to the
    // committed row.
...
pub(crate) async fn replace_owner_document_read_model(state: &AppState, stored: &StoredDocument) {
    // As above, a durable state must not duplicate persisted PDF/A bytes in an unbounded process
    // map. In-memory states still retain the newest owner slot plus displaced history.
    if state.store.is_some() {
        return;
    }

    let mut documents = state.documents.write().await;
    if let Some(displaced) = documents.get(&stored.act_id).cloned()
...
mod tests {
    ...
    #[test]
    fn effective_layout_resolver_applies_instance_template_entity_book_per_leaf() {
        use chancela_core::{
            DocumentLayoutOverrides, DocumentOrientation, DocumentPageLayoutOverrides,
            DocumentPageMarginsOverrides, DocumentRegionsOverrides, DocumentTypographyOverrides,
        };

        let mut instance = DocumentLayoutPolicy::default();
        instance.page.margins_mm.top = 18;
        instance.typography.body_font_size_pt = 11;

        let mut spec = registry()
            .get("csc-ata-ag/v1")
            .expect("spine template")
            .clone();
        spec.document_layout_override = Some(DocumentLayoutOverrides {
        ...
        entity.document_layout_override = Some(DocumentLayoutOverrides {
        ...
        book.document_layout_override = Some(DocumentLayoutOverrides {
            page: DocumentPageLayoutOverrides {
                margins_mm: DocumentPageMarginsOverrides {
                    left: Some(14),
                    ..Default::default()
        ...
        let resolved =
            resolve_effective_document_layout(&instance, &spec, Some(&entity), Some(&book))
                .expect("valid cascade");
        assert_eq!(resolved.page.margins_mm.top, 18, "instance survives");
        assert_eq!(
            resolved.typography.body_font_size_pt, 13,
            "entity overrides template"
        );
        assert_eq!(
            resolved.page.orientation,
            DocumentOrientation::Landscape,
            "entity supplies orientation"
        );
        assert_eq!(
            resolved.page.margins_mm.left, 14,
            "book overrides entity leaf"
        );
        assert_eq!(resolved.regions.footer_gap_mm, 8, "book supplies region");
    }

    #[test]
    fn generated_document_persists_and_event_binds_concrete_layout() {
        use chancela_core::{
            DocumentLayoutOverrides, DocumentPageLayoutOverrides, DocumentPageMarginsOverrides,
            DocumentRegionsOverrides,
        };

        let mut instance = DocumentLayoutPolicy::default();
        instance.page.margins_mm.top = 17;
        let mut entity = entity_of(EntityKind::SociedadeAnonima);
        entity.document_layout_override = Some(DocumentLayoutOverrides {
            page: DocumentPageLayoutOverrides {
                margins_mm: DocumentPageMarginsOverrides {
        ...
        book.document_layout_override = Some(DocumentLayoutOverrides {
            regions: DocumentRegionsOverrides {
                footer_gap_mm: Some(7),
                ..Default::default()
            },
            ..Default::default()
        });
        let act = Act::draft(book.id, "Ata", MeetingChannel::Physical);
        let spec = registry().get("csc-ata-ag/v1").expect("spine template");
        let expected =
            resolve_effective_document_layout(&instance, spec, Some(&entity), Some(&book))
                .expect("effective policy");

        let generated = generate_for_act(&act, &book, &entity, None, &instance)
            .expect("generation succeeds")
            .expect("spine exists");
        let stored_json = generated
            .stored
            .document_layout_json
            .as_deref()
            .expect("new documents bind a concrete layout");
        let stored_policy: DocumentLayoutPolicy =
        ...
        let recorded = generated.event_payload["document_layout_digest"]
            .as_str()
            .expect("event binds layout");
        assert_eq!(recorded, document_layout_digest_of(stored_json));
        assert!(
            generated
                .event_payload
                .get("template_spec_digest")
                .is_some(),
            "layout binding is additive and does not replace template identity"
        );
    }

    #[test]
    fn layout_digest_detects_changes_and_legacy_rows_remain_unbound() {
        let json =
            serde_json::to_string(&DocumentLayoutPolicy::default()).expect("layout serializes");
        let digest = document_layout_digest_of(&json);
        let mut changed = DocumentLayoutPolicy::default();
        changed.typography.body_font_size_pt = 12;
        let changed_json = serde_json::to_string(&changed).expect("changed layout serializes");
        assert_ne!(document_layout_digest_of(&changed_json), digest);

        let historical = StoredDocument {
            id: "historical-unbound".to_owned(),
        ...
        assert!(
            stored_document_layout(&historical)
                .expect("historical row parses")
                .is_none(),
    ...
    #[test]
    fn encerramento_replay_uses_frozen_layout_after_mutable_layers_change() {
        use chancela_core::{
            DocumentLayoutOverrides, DocumentOrientation, DocumentPageLayoutOverrides,
            DocumentTypographyOverrides,
        };
        ...
        let initial_instance = DocumentLayoutPolicy::default();

        let frozen =
            generate_encerramento_snapshot(&termo, &book, &entity, 7, Some(42), &initial_instance)
        ...
        let replay = generate_encerramento_snapshot_with_layout(
            &termo,
            &book,
            &entity,
            7,
        ...
        .expect("encerramento template");
        assert_eq!(
            replay.stored.pdf_digest, frozen.stored.pdf_digest,
            "mutable layout changes cannot alter the signed snapshot replay"
        );

        let mutable_rerender =
            generate_encerramento_snapshot(&termo, &book, &entity, 7, Some(42), &changed_instance)
                .expect("mutable rerender")
                .expect("encerramento template");
        assert_ne!(
            mutable_rerender.stored.pdf_digest, frozen.stored.pdf_digest,
            "the test must prove mutable policy really did move"
        );
    }
    ...
    async fn structural_preview_resolves_instance_then_draft_template_layout() {
        use chancela_core::{
            DocumentLayoutOverrides, DocumentOrientation, DocumentTypographyOverrides,
        };

        let state = AppState::default();
        {
            let mut settings = state.settings.write().await;
            settings.documents.layout_defaults.page.orientation = DocumentOrientation::Landscape;
            settings
                .documents
                .layout_defaults
                .typography
        ...
        let mut spec = registry()
            ...
            .expect("spine template")
            .clone();
        spec.id = "user-layout-preview/v1".to_owned();
        spec.document_layout_override = Some(DocumentLayoutOverrides {
            typography: DocumentTypographyOverrides {
                body_font_size_pt: Some(15),
                ..Default::default()
        ...
        let model = match prepared {
            Ok(model) => model,
            Err(response) => panic!("draft unexpectedly rejected with {}", response.status()),
        };
        assert_eq!(
            model.document_layout.page.orientation,
            DocumentOrientation::Landscape,
            "preview inherits current instance layout"
        );
        assert_eq!(
            model.document_layout.typography.body_font_size_pt, 15,
            "draft template override wins over instance"
        );
    }

    #[tokio::test]
    async fn scoped_act_preview_resolves_entity_and_book_layout_layers() {
        use chancela_core::{
            DocumentLayoutOverrides, DocumentOrientation, DocumentPageLayoutOverrides,
            DocumentRegionsOverrides,
        };
        ...
        {
            let mut settings = state.settings.write().await;
            settings.documents.layout_defaults.page.margins_mm.top = 18;
        }
        let mut entity = entity_of(EntityKind::SociedadeAnonima);
        entity.document_layout_override = Some(DocumentLayoutOverrides {
            page: DocumentPageLayoutOverrides {
        ...
        let Json(model) = preview_document(
            State(state),
            Path(act.id.0),
            actor,
            Query(PreviewQuery { template_id: None }),
        )
        .await
        .expect("scoped preview succeeds");
        assert_eq!(model.document_layout.page.margins_mm.top, 18);
        assert_eq!(
            model.document_layout.page.orientation,
            DocumentOrientation::Landscape
        );
        assert_eq!(model.document_layout.regions.footer_gap_mm, 9);
    }
    ...
    fn persist_document_fixture(state: &AppState, document: &StoredDocument) {
        state
            .store
            .as_ref()
            .expect("durable fixture store")
            .persist(|tx| tx.upsert_document(document))
            .expect("fixture document persists");
    }

    #[tokio::test]
    async fn durable_publish_does_not_retain_pdf_bytes_and_store_load_remains_available() {
        let tmp = TempDir::new();
        let state = AppState::with_data_dir(tmp.path());
        let (act_id, document, _) = export_fixture();
        persist_document_fixture(&state, &document);

        publish_generated_document_read_model(&state, &document).await;

        assert!(
            state.documents.read().await.is_empty(),
            "durable generated documents must not be duplicated in the process read model"
        );
        let loaded = load_document(&state, act_id)
            .await
            .expect("durable document load succeeds")
            .expect("persisted document remains available");
        assert_eq!(loaded.pdf_bytes, document.pdf_bytes);
    }

    #[tokio::test]
    async fn durable_replace_does_not_retain_pdf_bytes_and_store_load_remains_available() {
        let tmp = TempDir::new();
        let state = AppState::with_data_dir(tmp.path());
        let (act_id, document, _) = export_fixture();
        persist_document_fixture(&state, &document);

        replace_owner_document_read_model(&state, &document).await;

        assert!(
            state.documents.read().await.is_empty(),
            "durable owner replacement must not duplicate persisted PDF bytes"
        );
        let loaded = load_document(&state, act_id)
            .await
            .expect("durable document load succeeds")
    ...
    #[tokio::test]
    async fn in_memory_publish_still_caches_and_serves_pdf_bytes() {
        let state = AppState::default();
        let (act_id, document, _) = export_fixture();

        publish_generated_document_read_model(&state, &document).await;

        assert_eq!(state.documents.read().await.len(), 1);
        let loaded = load_document(&state, act_id)
            .await
            .expect("in-memory document load succeeds")
            .expect("published in-memory document remains available");
        assert_eq!(loaded.pdf_bytes, document.pdf_bytes);
    }

    #[tokio::test]
    async fn in_memory_replace_still_caches_and_serves_pdf_bytes() {
        let state = AppState::default();
        let (act_id, document, _) = export_fixture();
        ...
        let loaded = load_document(&state, act_id)
            .await
            .expect("in-memory document load succeeds")
            .expect("replaced in-memory document remains available");
        assert_eq!(loaded.pdf_bytes, document.pdf_bytes);
    }
    ...
    async fn certidao_generation_does_not_replace_canonical_ata_document() {
        ...
        let book = Book::new(entity.id, BookKind::AssembleiaGeral);
        let act = sealed_csc_act(&book);
        let ata = generate_for_act(&act, &book, &entity, None, &DocumentLayoutPolicy::default())
    ...
    async fn seed_absent_owner_dispatch_fixture(
        ...
        let ata = generate_for_act(&act, &book, &entity, None, &DocumentLayoutPolicy::default())
            ...
            .expect("ata document")
            .stored;
        let communication = generate_condominium_absent_owner_communication(
            &act,
            &book,
            &entity,
    ...
    async fn seed_generated_convening_dispatch_fixture(
        ...
        let ata = generate_for_act(&act, &book, &entity, None, &DocumentLayoutPolicy::default())
            ...
            .expect("ata document")
            .stored;
        let notice = generate_for_act_template(
            &act,
            &book,
            &entity,
            "csc-convocatoria-ag/v1",
    ...
    fn the_generated_event_carries_the_body_compiler_id_only_when_a_body_exists() {
        ...
        let without =
            generate_for_act(&act, &book, &entity, None, &DocumentLayoutPolicy::default())
                .expect("generates")
    ...
    fn a_whitespace_only_body_does_not_trip_the_placement_gate() {
        ...
        assert!(
            generate_for_act(&act, &book, &entity, None, &DocumentLayoutPolicy::default(),).is_ok(),
