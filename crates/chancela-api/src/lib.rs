...
mod tests {
    ...
    /// Response headers as sorted `(name, value)` pairs — see [`comparable_headers`].
    type ComparableHeaders = Vec<(String, String)>;

    /// One observed sign-in refusal: what was attempted, and everything the caller can see back.
    type ObservedRefusal = (&'static str, StatusCode, ComparableHeaders, Vec<u8>);

    /// Every response header, name and value, in a canonical order, for comparing two refusals.
    ///
    /// A `WWW-Authenticate` on one branch only, a differing `content-length`, or a cache directive
    /// attached to just one outcome is as good an enumeration oracle as a differing body, so nothing
    /// is filtered out — except `x-request-id`, whose value is a fresh per-request UUID that varies
    /// between any two calls whatsoever and carries nothing about the account. It is normalised to a
    /// placeholder rather than dropped, so the header still has to be *present* on both.
    fn comparable_headers(headers: &axum::http::HeaderMap) -> ComparableHeaders {
        let mut out: ComparableHeaders = headers
            .iter()
            .map(|(name, value)| {
                let value = String::from_utf8_lossy(value.as_bytes()).into_owned();
                if name.as_str() == "x-request-id" {
                    assert!(
                        Uuid::parse_str(&value).is_ok(),
                        "x-request-id is normalised away on the assumption it is an opaque \
                         per-request uuid, but it was {value:?}"
                    );
                    return (name.as_str().to_owned(), "<per-request>".to_owned());
                }
    ...
    /// The field paths inside `value` that hold an array or an object — the two shapes a list of
    /// users could arrive in, at any depth and under any name.
    fn container_paths(prefix: &str, value: &Value) -> Vec<String> {
        ...
        children
            .into_iter()
            .flat_map(|(path, child)| {
                let mut found = container_paths(&path, child);
    ...
    /// `GET /v1/session/roster` unauthenticated is **one boolean and nothing else**, asserted on the
    /// structure rather than on the absence of a known username from a serialized string. A future
    /// field that leaked usernames — under any name, in any shape — has to fail this: the key set is
    /// pinned exactly, the value must be a boolean, and no array or nested object may appear anywhere
    /// in the payload.
    ///
    /// The last assertion is the sharpest: two instances holding *different* users must serialize to
    /// **byte-identical** responses. Nothing about who exists can influence this endpoint's output,
    /// because its output does not vary with the users at all.
    #[tokio::test]
    async fn session_roster_answers_one_boolean_that_cannot_vary_with_who_exists() {
        /// The signed-out roster of an instance, as (status, raw bytes).
        async fn roster_of(state: &AppState) -> (StatusCode, Vec<u8>) {
            let (status, _, bytes) = send_raw_bytes(state.clone(), get("/v1/session/roster")).await;
            (status, bytes)
        }

        fn assert_shape(bytes: &[u8], expected_onboarding: bool) {
            let value: Value = serde_json::from_slice(bytes).expect("roster is JSON");
            let object = value.as_object().expect("roster is a JSON object");
            assert_eq!(
                object.keys().collect::<Vec<_>>(),
                vec!["onboarding_required"],
                "the unauthenticated roster must carry exactly one key: {value}"
            );
            assert!(
                object["onboarding_required"].is_boolean(),
                "onboarding_required must be a boolean, not a carrier for anything else: {value}"
            );
            assert_eq!(object["onboarding_required"], expected_onboarding);
            let containers = container_paths("roster", &value);
            assert!(
                containers.is_empty(),
                "the roster must hold no array and no nested object — user data can only arrive \
                 in one of those — but found {containers:?} in {value}"
            );
        }

        // A genuinely empty instance: onboarding is required, and even *that* answer is one boolean.
        let empty = fresh_state().await;
        let (status, bytes) = roster_of(&empty).await;
        assert_eq!(status, StatusCode::OK);
        assert_shape(&bytes, true);

        // The first-run bootstrap the flag gates is unaffected: an unauthenticated create succeeds on
        // the empty instance, and the flag then flips.
        let (status, amelia) = send_raw(
            empty.clone(),
            post_json(
                "/v1/users",
        ...
        .await;
        assert_eq!(status, StatusCode::CREATED, "bootstrap create: {amelia}");
        let (status, populated_bytes) = roster_of(&empty).await;
        assert_eq!(status, StatusCode::OK);
        assert_shape(&populated_bytes, false);

        // A second instance bootstrapped with a *different* user, then given a second user and an
        // inactive one, so the two instances agree on nothing except "somebody exists".
        let other = fresh_state().await;
        let (status, bruno) = send_raw(
            other.clone(),
            post_json(
                "/v1/users",
                json!({ "username": "bruno.ferreira", "password": DEFAULT_TEST_PASSWORD }),
            ),
        )
        .await;
        assert_eq!(status, StatusCode::CREATED);
        let token = open_session(&other, bruno["id"].as_str().expect("id")).await;
        let (status, carla) = send_raw(
            other.clone(),
            with_session(
                post_json(
                    "/v1/users",
                    json!({ "username": "carla.dias", "password": DEFAULT_TEST_PASSWORD }),
                ),
                &token,
            ),
        )
        ...
        let (status, _) = send_raw(
            ...
            with_session(
                patch_json(
                    &format!("/v1/users/{}", carla["id"].as_str().expect("id")),
                    json!({ "active": false }),
                ),
                &token,
            ),
        ...
        let (status, other_bytes) = roster_of(&other).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(
            other_bytes, populated_bytes,
            "two instances with entirely different users must return byte-identical rosters; a \
             difference here is user data leaking to an anonymous caller"
        );
    }

    /// The five ways sign-in refuses a caller who holds no valid credential must be **one** response,
    /// compared as raw bytes plus status plus headers — not five separate "it 401s" assertions, which
    /// would pass just as well against five subtly different bodies.
    ///
    /// The fifth case is an unguessable `user_id` that names nobody: the back-compat id path must not
    /// become the oracle the username path is not. (The *inactive* case is the one an implementation
    /// most easily leaks, since it is a real, resolvable user; it is covered here and in
    /// `create_session_refuses_unknown_inactive_wrong_password_and_empty_identically`, which pins the
    /// parsed bodies. This test adds the byte and header comparison.)
    #[tokio::test]
    async fn create_session_refusals_are_byte_identical_down_to_their_headers() {
        let (state, amelia_id) = state_with_amelia().await;
        let token = open_session(&state, &amelia_id).await;
        let (status, bruno) = send_raw(
            state.clone(),
        ...
        let (status, _) = send_raw(
            ...
            with_session(
                patch_json(
                    &format!("/v1/users/{}", bruno["id"].as_str().expect("id")),
                    json!({ "active": false }),
                ),
                &token,
            ),
        ...
        // Distinct identifiers throughout, so no two attempts share a backoff bucket and a 429 can
        // never masquerade as agreement between two refusals.
        let attempts = [
            (
                "unknown username",
                json!({ "username": "nao.existe", "password": DEFAULT_TEST_PASSWORD }),
            ...
            (
                "inactive user, with their own correct password",
                json!({ "username": "bruno", "password": DEFAULT_TEST_PASSWORD }),
            ),
            (
                "wrong password against a real active account",
            ...
            (
                "an unguessable user_id that names nobody",
                json!({ "user_id": Uuid::new_v4().to_string(), "password": DEFAULT_TEST_PASSWORD }),
            ),
        ];

        let mut observed: Vec<ObservedRefusal> = Vec::new();
        for (label, body) in attempts {
            let (status, headers, bytes) =
                send_raw_bytes(state.clone(), post_json("/v1/session", body)).await;
            assert_eq!(
                status,
                StatusCode::UNAUTHORIZED,
                "{label}: {}",
            ...
            observed.push((label, status, comparable_headers(&headers), bytes));
        }
        ...
        for (label, status, headers, bytes) in &observed[1..] {
            ...
            assert_eq!(
                *headers, reference_headers,
                "{label} differs in its response headers from an unknown username"
            );
            assert_eq!(
                String::from_utf8_lossy(bytes),
                String::from_utf8_lossy(&reference_bytes),
                "{label} is distinguishable byte-for-byte from an unknown username"
            );
        }
    }

    /// The throttle must not become the oracle the response body is not. It is keyed by user id, and
    /// an unknown identifier is folded into a synthetic bucket — so a *throttled* refusal has to look
    /// exactly like a throttled refusal for a real account. Otherwise an attacker stops reading the
    /// 401 and starts reading the 429: "this name gets speed-bumped, therefore it exists".
    ///
    /// Arming the speed-bump and then racing its 1 s expiry would make the outcome depend on how
    ...
    async fn create_session_throttle_refusal_is_the_same_for_real_and_unknown_identifiers() {
        ...
        let armed_until = time::OffsetDateTime::now_utc() + time::Duration::milliseconds(3_600_500);
        {
            ...
            assert_eq!(
                backoff.len(),
                2,
                "a real and an unknown identifier must occupy two distinct throttle buckets"
            );
            for entry in backoff.values_mut() {
        ...
        let throttled = |username: &'static str| {
            let state = state.clone();
            async move {
                let (status, headers, bytes) = send_raw_bytes(
                    state,
                    post_json(
                        "/v1/session",
                        json!({ "username": username, "password": "Palavra-Passe-Errada-9!" }),
                ...
                .await;
                (status, comparable_headers(&headers), bytes)
            }
        };

        let (real_status, real_headers, real_bytes) = throttled("amelia.marques").await;
        let (unknown_status, unknown_headers, unknown_bytes) = throttled("nao.existe").await;

        assert_eq!(real_status, StatusCode::TOO_MANY_REQUESTS);
        assert_eq!(
            unknown_status, real_status,
            "an unknown identifier must be throttled too, or its full-speed probing is the oracle"
        );
        assert_eq!(
            String::from_utf8_lossy(&real_bytes),
            String::from_utf8_lossy(&unknown_bytes),
    ...
    /// The dummy-verifier branch exists so an unknown identifier costs the same as a real one. Proven
    /// **structurally, never by the clock**: `attestation::verify_probe` logs every argon2 password
    /// verification actually performed on this thread, with the cost parameters that were really run.
    ///
    /// A wall-clock ratio would test the same property but flakes on a loaded machine, and a flaky
    /// test gets deleted by somebody later — taking the guarantee with it. This one is exact: the
    /// unknown path must run the same *number* of argon2 verifications, at *identical* m/t/p cost, as
    /// a wrong password against a real account. An early return logs zero and fails immediately.
    #[tokio::test]
    async fn create_session_runs_an_identical_argon2_verification_for_an_unknown_identifier() {
        let (state, _) = state_with_amelia().await;

        let attempt = |state: AppState, username: &'static str| async move {
            let _ = crate::attestation::verify_probe::take();
            let (status, _) = send_raw(
                state,
                post_json(
                    "/v1/session",
            ...
            .await;
            (status, crate::attestation::verify_probe::take())
        };

        // The dummy verifier is built once per process in a `OnceLock`; warm it so the measurement
        // below cannot be confused by first-use work. (Hashing is not verification, so it would not
        // be logged in any case — this only keeps the two attempts strictly comparable.)
        let (status, _) = attempt(state.clone(), "aquecimento.zzz").await;
        assert_eq!(status, StatusCode::UNAUTHORIZED);

        let (status, real) = attempt(state.clone(), "amelia.marques").await;
        assert_eq!(status, StatusCode::UNAUTHORIZED);
        assert_eq!(
            real.len(),
            1,
            "a wrong password against a real account must cost exactly one argon2 verification, \
             which is the baseline the unknown path has to match: {real:?}"
        );

        let (status, unknown) = attempt(state.clone(), "tambem.nao.existe").await;
        assert_eq!(status, StatusCode::UNAUTHORIZED);
        assert_eq!(
            unknown.len(),
            1,
            "an unknown identifier ran {} argon2 verifications instead of one — a short-circuit \
             here is a user-enumeration oracle read with a stopwatch: {unknown:?}",
            unknown.len()
        );

        // Same amount of work, and the same work: equal argon2 cost parameters, so "one call each"
        // cannot be satisfied by a deliberately cheap dummy.
        let (real, unknown) = (real[0], unknown[0]);
        assert_eq!(
            (unknown.m_cost, unknown.t_cost, unknown.p_cost),
            (real.m_cost, real.t_cost, real.p_cost),
            "the unknown path's argon2 cost differs from a real verification's, so the two are \
             still separable by timing: {unknown:?} vs {real:?}"
        );
        assert!(
            real.m_cost > 0 && real.t_cost > 0,
            "the probe read no cost parameters, so it is proving nothing: {real:?}"
        );
    }
