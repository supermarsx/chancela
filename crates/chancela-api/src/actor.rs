...
///
/// The same seam [`crate::roles::effective_permissions_for`] already uses: the whole resolver is a
/// pure function of `now`, and the *only* production entry point is [`resolve_session_actor`], which
/// passes `OffsetDateTime::now_utc()`. This is crate-private and takes no other argument, so it can
...
#[cfg(test)]
mod clock_tests {
    //! Expiry, renewal and revocation are all statements about *elapsed time*, so they can only be
    //! asserted honestly by moving the clock — backdating a stored timestamp instead proves the
    //! arithmetic, not the policy. [`resolve_session_actor_at`] is the seam that makes the clock an
    //! argument; production still has exactly one entry point, and it passes the real clock.
    ...
    use super::*;
    use crate::session::{DurableSessionRegistry, SESSIONS_FILE, SessionEntry};
    ...
    impl TempDir {
        fn new() -> Self {
            let path =
                std::env::temp_dir().join(format!("chancela-session-clock-{}", Uuid::new_v4()));
            std::fs::create_dir_all(&path).expect("create temp dir");
            Self(path)
        }

        /// Load the persisted registry as a process starting up at `now` would.
        fn registry_at(&self, now: OffsetDateTime) -> DurableSessionRegistry {
            DurableSessionRegistry::load_at(self.0.join(SESSIONS_FILE), now)
        }
    }
    ...
    /// A fixed epoch, so every assertion below is about a duration rather than about "now".
    fn t0() -> OffsetDateTime {
        OffsetDateTime::from_unix_timestamp(1_780_000_000).expect("valid timestamp")
    }

    async fn seed_user(state: &AppState) -> UserId {
    ...
    /// Put a live session in the node-local map exactly as minting does, issued at `issued_at`.
    async fn seed_live_session(state: &AppState, uid: UserId, issued_at: OffsetDateTime) -> String {
        ...
        state.sessions.write().await.insert(
            ...
            SessionEntry {
                user_id: uid,
                unlocked_key: None,
                expires_at: issued_at + Duration::seconds(SESSION_TTL_SECS),
            },
        );
        token
    }

    /// The sliding idle window is real elapsed time, not a stored number: a session untouched for
    /// longer than [`SESSION_TTL_SECS`] is refused, and one touched just inside it is not.
    #[tokio::test]
    async fn a_session_idle_past_the_sliding_window_is_refused() {
        let state = AppState::default();
        let uid = seed_user(&state).await;
        let token = seed_live_session(&state, uid, t0()).await;

        // One second short of the window: still good, and the window slides forward from there.
        assert_eq!(
            resolve_session_actor_at(
                &state,
                &token,
                t0() + Duration::seconds(SESSION_TTL_SECS - 1)
            )
            .await
        ...
        // A second session that is simply never used again falls off the far side of the window.
        let idle = seed_live_session(&state, uid, t0()).await;
        assert_eq!(
            resolve_session_actor_at(&state, &idle, t0() + Duration::seconds(SESSION_TTL_SECS))
                .await
                .expect("resolver succeeds"),
            None,
            "an idle session must not survive its own expiry instant",
        );
        assert!(
            !state.sessions.read().await.contains_key(&idle),
            "the expired entry is dropped rather than left resolvable",
        );
    }

    /// The absolute cap is the bound the sliding window cannot buy its way out of: a session used
    /// continuously — every renewal landing well inside the idle window — is still refused once its
    /// total age reaches the cap.
    #[tokio::test]
    async fn a_continuously_active_session_is_still_refused_at_the_absolute_cap() {
        let state = AppState::default();
        let cap = Duration::seconds(state.session_max_lifetime.0);
        let uid = seed_user(&state).await;
        let token = seed_live_session(&state, uid, t0()).await;

        // Used every 20 hours — never idle for the full 24 — right up to the cap.
        let mut elapsed = Duration::ZERO;
        while elapsed + Duration::hours(20) < cap {
            elapsed += Duration::hours(20);
            assert_eq!(
                resolve_session_actor_at(&state, &token, t0() + elapsed)
                    .await
                    .expect("resolver succeeds")
                    .as_deref(),
                Some("amelia.marques"),
                "an actively used session must not be logged out before the cap (at {elapsed})",
            );
        }

        // The idle window is wide open here — the session was used less than 20 hours ago — and the
        // session is refused anyway, which is the entire point of the cap.
        assert_eq!(
            resolve_session_actor_at(&state, &token, t0() + cap)
                .await
                .expect("resolver succeeds"),
            None,
            "renewing forever must not outlive the absolute cap",
        );
        assert!(!state.sessions.read().await.contains_key(&token));
        assert!(!state.session_issued_at.read().await.contains_key(&token));
    }

    /// Revocation is immediate, not deferred to the next expiry: a token signed out mid-window stops
    /// resolving from that moment, and the durable record goes with it.
    #[tokio::test]
    async fn revocation_takes_effect_immediately_inside_the_idle_window() {
        let temp = TempDir::new();
        let state = AppState {
            durable_sessions: temp.registry_at(t0()),
            ..AppState::default()
        };
        let uid = seed_user(&state).await;
        let token = seed_live_session(&state, uid, t0()).await;
        ...
        assert!(
            resolve_session_actor_at(&state, &token, t0() + Duration::hours(1))
                .await
                .expect("resolver succeeds")
                .is_some(),
            "the session is valid before it is revoked",
        );

        let mut headers = axum::http::HeaderMap::new();
        headers.insert(
            SESSION_HEADER,
            token.parse().expect("token is a header value"),
        );
        crate::session::delete_session(axum::extract::State(state.clone()), headers)
            .await
            .expect("sign-out succeeds");

        // Still hours inside both the idle window and the cap — and still refused.
        assert_eq!(
            resolve_session_actor_at(&state, &token, t0() + Duration::hours(2))
                .await
                .expect("resolver succeeds"),
            None,
            "a revoked token must not resolve again inside its window",
        );
        // And it stays refused after a restart, i.e. the revocation reached the persisted record.
        let restarted = AppState {
            durable_sessions: temp.registry_at(t0() + Duration::hours(3)),
            ..AppState::default()
        };
        seed_user(&restarted).await;
        assert_eq!(
            resolve_session_actor_at(&restarted, &token, t0() + Duration::hours(3))
                .await
                .expect("resolver succeeds"),
            None,
            "revocation must survive a restart, not be re-animated from disk",
        );
    }

    /// A persisted session survives a restart — that is what the durable registry is for — but it
    /// inherits both bounds. Neither the idle window nor the absolute cap can be reset by restarting
    /// the process, which is exactly what a stolen `sessions.json` token would try.
    #[tokio::test]
    async fn a_persisted_token_cannot_outlive_either_bound() {
        let temp = TempDir::new();
        let uid = UserId(Uuid::new_v4());

        // A fresh process each time: only the persisted digest record carries the session forward.
        async fn restart(temp: &TempDir, uid: UserId, now: OffsetDateTime) -> AppState {
            let state = AppState {
                durable_sessions: temp.registry_at(now),
                ..AppState::default()
            };
        ...
        // --- The idle bound survives the restart.
        let idle_token = Uuid::new_v4().to_string();
        let first = restart(&temp, uid, t0()).await;
        first
            .durable_sessions
            .insert(
                &idle_token,
            ...
            .await
            .expect("durable insert");
        let after_restart = restart(&temp, uid, t0() + Duration::hours(23)).await;
        assert_eq!(
            resolve_session_actor_at(
                &after_restart,
                &idle_token,
            ...
            .expect("resolver succeeds"),
            None,
            "a persisted token past its idle window must not be re-animated by a restart",
        );

        // --- The absolute cap survives the restart too, even with the idle window kept fresh by
        //     use. Each iteration is a new process resolving only from disk.
        let token = Uuid::new_v4().to_string();
        let seeding = restart(&temp, uid, t0()).await;
        seeding
            .durable_sessions
            .insert(
                &token,
            ...
            .await
            .expect("durable insert");
        let cap = Duration::seconds(seeding.session_max_lifetime.0);

        let mut elapsed = Duration::ZERO;
        while elapsed + Duration::hours(20) < cap {
            elapsed += Duration::hours(20);
            let restarted = restart(&temp, uid, t0() + elapsed).await;
            assert!(
                resolve_session_actor_at(&restarted, &token, t0() + elapsed)
                    .await
                    .expect("resolver succeeds")
                    .is_some(),
                "a persisted, actively used session survives restarts before the cap (at {elapsed})",
            );
        }

        let restarted = restart(&temp, uid, t0() + cap).await;
        assert_eq!(
            resolve_session_actor_at(&restarted, &token, t0() + cap)
                .await
                .expect("resolver succeeds"),
            None,
            "the absolute cap is anchored to the persisted issue time, not to process start",
        );
        // The over-age record is dropped from disk, so no later restart can resurrect it either.
        let final_state = restart(&temp, uid, t0() + cap + Duration::seconds(1)).await;
        assert_eq!(
            resolve_session_actor_at(&final_state, &token, t0() + cap + Duration::seconds(1))
                .await
                .expect("resolver succeeds"),
            None,
        );
