...
pub(crate) const ROUTE_CLASSIFICATION: &[(&str, RouteClass)] = &[
    ...
    ("/v1/tenants/{tenant_id}", RouteClass::Gated), // GET tenant.read@Tenant · PATCH tenant.admin@Tenant
    ...
    (
        ...
        RouteClass::Gated,
    ), // POST book.close@Book (freeze)
    ("/v1/books/{id}/termo/encerramento/sign", RouteClass::Gated), // POST book.close@Book (collect signature reference)
    (
    ...
    ("/v1/templates/document/preview/markdown", RouteClass::Gated), // POST act.read@Global (same unresolved model rendered as Markdown, never writes)
    // --- Ledger ---------------------------------------------------------------------------------
