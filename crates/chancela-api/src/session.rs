...
impl DurableSessionRegistry {
    ...
    /// [`load`](Self::load) with the wall clock supplied explicitly, so a test can observe a restart
    /// at a chosen instant. Startup discards already-expired records, which is a statement about
    /// elapsed time; the only production caller is [`load`](Self::load) and it passes the real clock.
    /// Crate-private, one extra argument, and it cannot loosen anything — a later `now` discards
    /// *more* records, never fewer.
    pub(crate) fn load_at(path: PathBuf, now: OffsetDateTime) -> Self {
        let records = load_durable_sessions(&path, now).unwrap_or_else(|error| {
            eprintln!(
...
fn load_durable_sessions(
    path: &Path,
    now: OffsetDateTime,
) -> Result<HashMap<String, DurableSessionRecord>, String> {
