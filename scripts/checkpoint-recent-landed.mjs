...
function assertCheckpointMap() {
  ...
  // t33 replaced the roster picker with a typed identifier, and t33-e2 removed the
  // unauthenticated roster entirely: there is no "every roster user" to prompt for, so the
  // password-prompt guarantee is now pinned on the typed sign-in itself.
  assertFileContains(
    "apps/web/src/features/session/session.test.tsx",
    "a typed identifier + correct password signs in",
    "web sign-in password prompt coverage",
  );
  assertFileContains(
    "apps/web/src/features/session/session.test.tsx",
    "bootstrap: no user yet → create one with a password → sign-in lands in the app",
  ...
  // t33-e2: the unauthenticated roster no longer reports `has_secret` (or any user), so the
  // reset can no longer pre-check that the operator has a verifier — it opens a real password
  // session by identifier and lets the server's single opaque failure speak.
  assertFileContains(
    "apps/web/e2e/fixtures.ts",
    "username: OPERATOR.username,",
    "browser e2e reset opens a password session by identifier marker",
  );
  assertFileContains(
    "apps/web/e2e/session.spec.ts",
    "settings-created users require passwords and can sign in with that password",
  ...
  assertFileContains(
    "docs/CI-CHECKPOINTS.md",
    "The current substantive checkpoint is `2f700c1` (2026-07-26)",
  ...
  assertFileContains(
    ...
    "ledger archive all-filtered PDF/A cap marker",
  );
  assertFileContains(
    "crates/chancela-api/src/arquivo.rs",
  ...
  assertFileContains(
    "crates/chancela-api/src/arquivo.rs",
    "stream_all_filtered_interchange_export(",
    "ledger archive all-filtered streaming interchange marker",
  );
  assertFileContains(
    "crates/chancela-api/src/arquivo.rs",
    "No records were truncated.",
    "ledger archive all-filtered PDF/A over-cap no-truncation marker",
  );
  assertFileContains(
    "crates/chancela-api/src/arquivo.rs",
  ...
  assertFileContains(
    "crates/chancela-api/src/lib.rs",
    "ledger_archive_document_streams_all_filtered_audit_interchange_formats",
    "ledger archive all-filtered streamed interchange coverage",
  );
  assertFileContains(
    "crates/chancela-api/src/lib.rs",
    "ledger_archive_document_caps_all_filtered_pdfa_without_truncating",
    "ledger archive all-filtered PDF/A cap coverage",
  );
  assertFileContains(
    "crates/chancela-api/src/lib.rs",
    'assert_eq!(export["streaming_mode"], "streamed");',
    "ledger archive all-filtered streaming metadata coverage",
  );
  assertFileContains(
    "apps/web/src/i18n/locales/en-US.ts",
    "all filtered JSON/TXT/CSV/HTML streams on the server",
    "ledger archive all-filtered help copy streaming marker",
  );
  assertFileContains(
  ...
  assertFileContains(
    "docs/CI-E2E-HARDENING-PLAN.md",
    "Current `040ce48` all-filtered archive export checks",
  ...
  assertFileContains(
    ...
    "db_key_env_matches_runtime_sqlcipher_capability_without_leaking_key",
    "CLI database key runtime capability coverage",
  );
  assertFileContains(
    "crates/chancela-store/tests/store.rs",
    "operator_status_text_normalization_and_sqlite_cluster_seams_are_complete_and_secret_free",
    "store operator status and SQLite seam coverage",
  );
  assertFileContains(
    "crates/chancela-store/tests/recovery.rs",
    "let wrong = store.restore_encrypted(",
    "store encrypted restore wrapper coverage",
  );
  assertFileContains(
    "crates/chancela-cli/tests/cli.rs",
  ...
  assertFileContains(
    "SPEC-COVERAGE.md",
    "implementation snapshot `2f700c1e10e1cf3fbe6b3b002ad9aac3c94040e3`",
  ...
  assertFileContains(
    "docs/CI-E2E-HARDENING-PLAN.md",
    "Snapshot drift/test-determinism checkpoint through `c9cf2cb`",
    "CI/E2E hardening plan snapshot drift/test determinism marker",
  );
  assertFileContains(
    "docs/CI-CHECKPOINTS.md",
    "Current snapshot drift/test-determinism markers additionally record only\n`fafb8ad` mkdocs navigation drift capture",
    "CI checkpoints snapshot drift/test determinism marker",
  );
  assertFileContains(
    "docs/CI-E2E-HARDENING-PLAN.md",
    "Current checkpoint metadata/static checks through `2f700c1`",
  ...
  assertFileContains(
    "docs/CI-E2E-HARDENING-PLAN.md",
    "SQLite remains the default, Postgres selection is\n  feature/config gated",
    "CI/E2E hardening plan database backend default/gated marker",
  );
  assertFileContains(
    "docs/CI-E2E-HARDENING-PLAN.md",
  ...
  assertFileContains(
    ...
    "AppState database backend resolution call marker",
  );
  assertFileContains(
    "crates/chancela-api/src/lib.rs",
    "match Store::open_backend(resolved_backend.selection)",
    "AppState database backend open marker",
  );
  assertFileContains(
  ...
  assertFileContainsNormalized(
    "docs/CI-CHECKPOINTS.md",
    "The prior `c9c1b19` `cargo test --workspace --locked` gate also exited 0 in 1136.5 seconds; no aggregate test count is inferred from Cargo's per-target output. That result is not relabelled as full-workspace validation of `2f700c1`",
  ...
  assertFileContainsNormalized(
    "docs/CI-E2E-HARDENING-PLAN.md",
    "The prior `c9c1b19` `cargo test --workspace --locked` gate exited 0 in 1136.5 seconds; no aggregate test count is inferred from Cargo's per-target output. That result is not relabelled as full-workspace validation of `2f700c1`",
